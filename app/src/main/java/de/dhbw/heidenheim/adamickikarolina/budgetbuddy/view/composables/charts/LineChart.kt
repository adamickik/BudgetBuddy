...
fun LineChart(
    ...
    val currentValue by analyticsViewModel.getSumOfExpensesByAssignmentID(assignmentId = savingGoal.sgId!!).observeAsState(0)
    val points by analyticsViewModel.getCumulativeAmountsByAssignmentId(savingGoal.sgId!!).observeAsState(initial = emptyList())
    val maxValue by analyticsViewModel.getMaxCumulativeAmountByAssignmentId(savingGoal.sgId!!).observeAsState(initial=0f)
